...
contract RegisterL1Bridge is Script {
  ...
  function run() public {
    ...
    address l1Bridge = json.readAddress(".deployments[0].transactions[1].contractAddress");

    vm.broadcast();
	IERC20Receive(address(l2ERC20)).registerApplicationContracts(targetChain, bytes32(uint256(uint160(address(l1Bridge)))));
